...
function ChatModule({user}){
  ...
  // COO: executive level — only chats with HR + Stores Admin (peer staff, not dept heads)
  if(isCOO){
    if(view==="list")
...
function COOChatList({user,onSelect}){
  ...
  const peers=["admin","hr"]; // COO only chats with Stores Admin + HR
  useEffect(()=>{
